...
/**
 * Created by Yzq on 2018/9/7 0007 11:12
...
public class EasyIndicator extends LinearLayout implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    public void setTabTitles(String[] tabTitles) {
        ...
        if (indicator_isBottom_line) {
            ...
            mMeasureText = (int) tvs[0].getPaint().measureText("用户评价");
            indicator.setLayoutParams(new LinearLayoutCompat.LayoutParams(mMeasureText, indicator_bottom_height));
            // Executive animation
            tvs[0].post(new Runnable() {
                @Override
                public void run() {
                    buildIndicatorAnimatorTowards(tvs[0],false).start();
                }
            });
    ...
    private AnimatorSet buildIndicatorAnimatorTowards(TextView tv,boolean isQuick) {
        float x = tab_content.getX();
        ObjectAnimator xAnimator = ObjectAnimator.ofFloat(indicator, "X", indicator.getX(), tv.getX() + (tv.getMeasuredWidth() - mMeasureText) /2);
        final ViewGroup.LayoutParams params = indicator.getLayoutParams();
        ValueAnimator widthAnimator = ValueAnimator.ofInt(50, 50);
    ...
    public void onClick(View v) {
        ...
        if (onTabClickListener != null) {
            ...
            if (viewPager != null) {
                ...
                setSelectorColor(tv);
                if (indicator_isBottom_line) {
                    buildIndicatorAnimatorTowards(tv,false).start();
                }
            }
    ...
    private void setSelectorColor(TextView tv) {
        ...
        if (indicator_isBottom_line) {
            buildIndicatorAnimatorTowards(tv,true).start();
        }
    }
