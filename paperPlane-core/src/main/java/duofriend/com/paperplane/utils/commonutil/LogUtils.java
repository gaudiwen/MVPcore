...
import duofriend.com.paperplane.BuildConfig;
...
public final class LogUtils {
    ...
    private static boolean sLogSwitch         = BuildConfig.SWITCH_LOG; // log总开关，默认开
    ...
    /**
     *    要去配置.withLogcatSwitch(true)
     * @param type
     * @param tag
     * @param contents
     */
    private static void log(final int type, final String tag, final Object... contents) {
        if(Plane.getConfigurations().get(ConfigType.LOGCAT_STITCH.name()) != null){
            if (!(boolean)Plane.getConfigurations().get(ConfigType.LOGCAT_STITCH.name())) {
                return;
            }
        }
