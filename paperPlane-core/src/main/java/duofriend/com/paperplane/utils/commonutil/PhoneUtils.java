...
public final class PhoneUtils {
  ...
  /*  @SuppressLint("HardwareIds")
    public static String getDeviceUniqueID() {
        String deviceUniqueID = Hawk.get("DeviceUniqueID");
        if (!judgeIllegalCode(deviceUniqueID)) {
            TelephonyManager tm = (TelephonyManager) Utils.getContext().getSystemService(Context.TELEPHONY_SERVICE);
            if (tm != null && judgeIllegalCode(tm.getDeviceId())) {
                Hawk.put("DeviceUniqueID", tm.getDeviceId());
                LogUtils.d("tm.getDeviceId()=" + tm.getDeviceId());
                return tm.getDeviceId();
            } else {//如果IMEI码无效，取串码
                String SerialNumber = android.os.Build.SERIAL;
                if (judgeIllegalCode(SerialNumber)) {
                    Hawk.put("DeviceUniqueID", SerialNumber);
                    LogUtils.d("SerialNumber=" + SerialNumber);
                    return SerialNumber;
                } else {//如果串码无效，取WIFI MAC地址
                    ...
                    if (wifi != null && wifi.getConnectionInfo() != null) {
                        if (!("00:00:00:00:00:00").equals(wifi.getConnectionInfo().getMacAddress())
                                && !("02:00:00:00:00:00").equals(wifi.getConnectionInfo().getMacAddress())) {
                            WifiInfo info = wifi.getConnectionInfo();
                            String wifiMac = info.getMacAddress().replaceAll(":", "");
                            Hawk.put("DeviceUniqueID", wifiMac);
                            LogUtils.d("wifiMac=" + wifiMac);
                            return wifiMac;
                        } else {
                            String wifiMac = WifiUtil4Marshmallow.getwifiMacAddress();
                            if (judgeIllegalCode(wifiMac)) return wifiMac;
                        }
    ...
    public static boolean judgeIllegalCode(String code){
        if (!TextUtils.isEmpty(code)&&(!(code.equals("0") || code.equals("0123456789abcdef")
                || code.equals("000000000000000") ||code.equals("null") || code.equals("NULL")
                ||code.equals("00:00:00:00:00:00")||code.equals("02:00:00:00:00:00")
                ||code.equals("UNKNOWN")||code.equals("unknown")
        ))){
            return true;
        }
        return false;
